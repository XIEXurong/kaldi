...
if [ "$oldlm" == "$oldlang/G.fst" ]; then
  for JOB in `seq 1 $nj`; do
	echo "lattice-lmrescore --lm-scale=$oldlm_weight \
    ...
    lattice-reverse ark:- \"ark,t:|gzip -c>$outdir/lat.$JOB.gz\" || exit 1;" > $outdir/log/$JOB.sh
	$srun_cmd bash $outdir/log/$JOB.sh &> $outdir/log/rescorelm.$JOB.log &
  done
  wait
else
  for JOB in `seq 1 $nj`; do
	echo "lattice-lmrescore-const-arpa --lm-scale=$oldlm_weight \
