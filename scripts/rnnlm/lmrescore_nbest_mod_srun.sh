...
if [ ! -z "$num_jobs" ]; then
    nj=$num_jobs
	echo "$num_jobs" > $dir/num_jobs || exit 1;
    
    mkdir -p $dir/old_lat
	nj_old=`cat $indir/num_jobs` || exit 1;
	for JOB in `seq 1 $nj_old`; do
		lattice-push --push-strings=false --push-weights=false "ark:gunzip -c $indir/lat.$JOB.gz|" ark,scp:$dir/old_lat/$JOB.lat,$dir/old_lat/$JOB.lat.scp &
	done
	wait
	
	cat $dir/old_lat/*.lat.scp > $dir/old_lat/all_lat.scp
	splitFile.sh $dir/old_lat/all_lat.scp $nj
else
  ...
  if [ ! -z "$num_jobs" ]; then
      if $keep_ali; then
        for JOB in `seq 1 $nj`; do
          ( $srun_cmd lattice-to-nbest --acoustic-scale=$acwt --n=$N \
          scp:$dir/old_lat/all_lat_sub$JOB.scp \
          "ark:|gzip -c >$dir/nbest1.$JOB.gz" &> $dir/log/lat2nbest.$JOB.log || exit 1; ) &
        done
        wait
      else
        for JOB in `seq 1 $nj`; do
          ( $srun_cmd lattice-to-nbest --acoustic-scale=$acwt --n=$N \
          scp:$dir/old_lat/all_lat_sub$JOB.scp ark:- \|  \
          lattice-rmali ark:- "ark:|gzip -c >$dir/nbest1.$JOB.gz" &> $dir/log/lat2nbest.$JOB.log || exit 1; ) &
        done
        wait
      fi
  else
