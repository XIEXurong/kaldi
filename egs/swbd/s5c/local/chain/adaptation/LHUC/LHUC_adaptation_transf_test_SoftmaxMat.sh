...
cat <<EOF >> $dir/configs/change.config	
	component name=LHUC.linear.tied.pred type=LinearSelectColComponent input-dim=1 output-dim=$num_cols_mat col-num=$spk_num l2-regularize=0.00 param-mean=$param_init param-stddev=0 use-natural-gradient=false learning-rate-factor=${adapt_lr_factor}
	component-node name=LHUC.linear.tied.pred component=LHUC.linear.tied.pred input=feature2
    component name=LHUC.linear.tied.pred_softmax type=SoftmaxComponent dim=$num_cols_mat
    component-node name=LHUC.linear.tied.pred_softmax component=LHUC.linear.tied.pred_softmax input=LHUC.linear.tied.pred
    component name=LHUC.linear.tied type=LinearComponent input-dim=$num_cols_mat output-dim=$adapt_dim learning-rate-factor=0 matrix=$lab_mat
    component-node name=$adapt_input.tied$adapt component=LHUC.linear.tied input=LHUC.linear.tied.pred_softmax
EOF
fi
...
cat <<EOF >> $dir/configs/change.config	
    component name=LHUC.linear.$layer.pred type=LinearSelectColComponent input-dim=1 output-dim=$num_cols_mat col-num=$spk_num l2-regularize=0.00 param-mean=$param_init param-stddev=0 use-natural-gradient=false learning-rate-factor=${adapt_lr_factor}
	component-node name=LHUC.linear.$layer.pred component=LHUC.linear.$layer.pred input=feature2
    component name=LHUC.linear.$layer.pred_softmax type=SoftmaxComponent dim=$num_cols_mat
    component-node name=LHUC.linear.$layer.pred_softmax component=LHUC.linear.$layer.pred_softmax input=LHUC.linear.$layer.pred
    component name=LHUC.linear.$layer type=LinearComponent input-dim=$num_cols_mat output-dim=$adapt_dim learning-rate-factor=0 matrix=$lab_mat
    component-node name=$adapt_input.$layer$adapt component=LHUC.linear.$layer input=LHUC.linear.$layer.pred_softmax
EOF
fi
...
if [ -z $adapt_model ]; then
    adapt_model=$dirbase/$model_iter.mdl
fi
