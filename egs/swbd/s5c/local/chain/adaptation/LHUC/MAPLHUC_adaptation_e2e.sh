...
version=_MAPLHUC_e2e${tag}_adaptlayer${layer_num}_act${act}_epoch${epoch_num}_lr1${lr1}_lr2${lr2}
...
if [[ "$i" -lt "${#prior_mean_array[*]}" || "$i" -lt "${#prior_mean_file_array[*]}" || "$i" -lt "${#prior_std_array[*]}" || "$i" -lt "${#prior_std_file_array[*]}" ]]; then
  prior_layer=$layer
cat <<EOF >> $dir/configs/change.config
	component name=MAPLHUC.prior_mean.$prior_layer type=ConstantFunctionComponent input-dim=1 is-updatable=false output-dim=$dim_tmp $prior_mean_config
	component-node name=MAPLHUC.prior_mean.$prior_layer component=MAPLHUC.prior_mean.$prior_layer input=feature2
	component name=MAPLHUC.prior_std.$prior_layer type=ConstantFunctionComponent input-dim=1 is-updatable=false output-dim=$dim_tmp $prior_std_config
	component-node name=MAPLHUC.prior_std.$prior_layer component=MAPLHUC.prior_std.$prior_layer input=feature2
EOF
fi

cat <<EOF >> $dir/configs/change.config	
	component name=MAPLHUC.mean.$layer type=LinearSelectColComponent input-dim=1 output-dim=$dim_tmp col-num=$spk_num l2-regularize=0.00 param-mean=$param_mean_init param-stddev=0 use-natural-gradient=false
	component-node name=MAPLHUC.mean.$layer component=MAPLHUC.mean.$layer input=feature2
EOF
...
# unused
cat <<EOF >> $dir/configs/change.config	
	component name=MAPLHUC.std_ori.$layer type=LinearSelectColComponent input-dim=1 output-dim=$tie_dim col-num=$spk_num l2-regularize=0.00 param-mean=$param_std_init param-stddev=0 use-natural-gradient=false learning-rate-factor=0
	component-node name=MAPLHUC.std_ori.$layer component=MAPLHUC.std_ori.$layer input=feature2
EOF
...
if [[ "$log_std" == "true" ]]; then
  std_ori=std_exp
cat <<EOF >> $dir/configs/change.config
    component name=MAPLHUC.std_exp.$layer type=ExpComponent dim=$tie_dim self-repair-scale=0
	component-node name=MAPLHUC.std_exp.$layer component=MAPLHUC.std_exp.$layer input=MAPLHUC.std_ori.$layer
EOF
fi

cat <<EOF >> $dir/configs/change.config	
	component name=MAPLHUC.std_scale.$layer type=NoOpComponent dim=$tie_dim backprop-scale=$std_update_scale
	component-node name=MAPLHUC.std_scale.$layer component=MAPLHUC.std_scale.$layer input=MAPLHUC.$std_ori.$layer
	component name=MAPLHUC.std.$layer type=CopyNComponent input-dim=$tie_dim output-dim=$dim_tmp
	component-node name=MAPLHUC.std.$layer component=MAPLHUC.std.$layer input=MAPLHUC.std_scale.$layer
EOF

cat <<EOF >> $dir/configs/change.config	
	component name=MAPLHUC.vec.$layer type=BayesVecKLGaussianComponent output-dim=$dim_tmp input-dim=$dim_tmp_x4 KL-scale=${KL} input-frame-scale=false rand-per-frame=false KL-output=false test-mode=false fix-rand-val=true
	component-node name=MAPLHUC.vec.$layer component=MAPLHUC.vec.$layer input=Append(MAPLHUC.mean.$layer, MAPLHUC.std.$layer, MAPLHUC.prior_mean.$prior_layer, MAPLHUC.prior_std.$prior_layer)
	component name=MAPLHUC.act.$layer $act_component dim=$dim_tmp
	component-node name=MAPLHUC.act.$layer component=MAPLHUC.act.$layer input=MAPLHUC.vec.$layer
	component name=MAPLHUC.product.$layer type=ElementwiseProductComponent output-dim=$dim_tmp input-dim=$dim_tmp_x2
	component-node name=MAPLHUC.product.$layer component=MAPLHUC.product.$layer input=Append($layer.relu, Scale($lhuc_scale, MAPLHUC.act.$layer))
	component-node name=$layer.batchnorm component=$layer.batchnorm input=MAPLHUC.product.$layer
EOF
