...
version=_BLHUC${tag}_adaptlayer${layer_num}_act${act}_batch${num_chunk1}_epoch${epoch_num}_lr1${lr1}_lr2${lr2}
...
dim_tmp_x4plus1=$(awk "BEGIN{print(4*$adapt_dim+1)}")
KL=`echo ${KL_scale_array[0]}`

prior_mean_tmp=`echo ${prior_mean_array[0]}`
prior_mean_config="output-mean=$prior_mean_tmp output-stddev=0"
if [ ! -z $prior_mean_file ]; then
  prior_mean_file_tmp=`echo ${prior_mean_file_array[0]}`
  prior_mean_config="vector=$prior_mean_file_tmp"
fi
prior_std_tmp=`echo ${prior_std_array[0]}`
prior_std_config="output-mean=$prior_std_tmp output-stddev=0"
if [ ! -z $prior_std_file ]; then
  prior_std_file_tmp=`echo ${prior_std_file_array[0]}`
  prior_std_config="vector=$prior_std_file_tmp"
fi

cat <<EOF >> $dir/configs/change.config
	component name=BLHUC.prior_mean.tied type=ConstantFunctionComponent input-dim=1 is-updatable=false output-dim=$adapt_dim $prior_mean_config
	component-node name=BLHUC.prior_mean.tied component=BLHUC.prior_mean.tied input=feature2
	component name=BLHUC.prior_std.tied type=ConstantFunctionComponent input-dim=1 is-updatable=false output-dim=$adapt_dim $prior_std_config
	component-node name=BLHUC.prior_std.tied component=BLHUC.prior_std.tied input=feature2
EOF

cat <<EOF >> $dir/configs/change.config	
	component name=BLHUC.mean.tied type=LinearSelectColComponent input-dim=1 output-dim=$adapt_dim col-num=$spk_num l2-regularize=0.00 param-mean=$param_mean_init param-stddev=0 use-natural-gradient=false learning-rate-factor=${adapt_lr_factor}
	component-node name=BLHUC.mean.tied component=BLHUC.mean.tied input=feature2
EOF
...
cat <<EOF >> $dir/configs/change.config	
	component name=BLHUC.std_ori.tied type=LinearSelectColComponent input-dim=1 output-dim=$tie_dim col-num=$spk_num l2-regularize=0.00 param-mean=$param_std_init param-stddev=0 use-natural-gradient=false learning-rate-factor=${adapt_lr_factor}
	component-node name=BLHUC.std_ori.tied component=BLHUC.std_ori.tied input=feature2
EOF
...
if [[ "$log_std" == "true" ]]; then
  std_ori=std_exp
cat <<EOF >> $dir/configs/change.config
    component name=BLHUC.std_exp.tied type=ExpComponent dim=$tie_dim self-repair-scale=0
	component-node name=BLHUC.std_exp.tied component=BLHUC.std_exp.tied input=BLHUC.std_ori.tied
EOF
fi

cat <<EOF >> $dir/configs/change.config	
	component name=BLHUC.std_scale.tied type=NoOpComponent dim=$tie_dim backprop-scale=$std_update_scale
	component-node name=BLHUC.std_scale.tied component=BLHUC.std_scale.tied input=BLHUC.$std_ori.tied
	component name=BLHUC.std.tied type=CopyNComponent input-dim=$tie_dim output-dim=$adapt_dim
	component-node name=BLHUC.std.tied component=BLHUC.std.tied input=BLHUC.std_scale.tied
EOF

cat <<EOF >> $dir/configs/change.config	
	component name=BLHUC.vec.tied type=BayesVecKLGaussianComponent output-dim=$adapt_dim input-dim=$dim_tmp_x4plus1 KL-scale=${KL} input-frame-scale=true inv-frame-scale=true rand-per-frame=false KL-output=false test-mode=false
	component-node name=$adapt_input.tied$adapt_op component=BLHUC.vec.tied input=Append(BLHUC.mean.tied, BLHUC.std.tied, BLHUC.prior_mean.tied, BLHUC.prior_std.tied, BLHUC.count)
EOF
...
if [[ "$i" -lt "${#prior_mean_array[*]}" || "$i" -lt "${#prior_mean_file_array[*]}" || "$i" -lt "${#prior_std_array[*]}" || "$i" -lt "${#prior_std_file_array[*]}" ]]; then
  prior_layer=$layer
cat <<EOF >> $dir/configs/change.config
	component name=BLHUC.prior_mean.$prior_layer type=ConstantFunctionComponent input-dim=1 is-updatable=false output-dim=$adapt_dim $prior_mean_config
	component-node name=BLHUC.prior_mean.$prior_layer component=BLHUC.prior_mean.$prior_layer input=feature2
	component name=BLHUC.prior_std.$prior_layer type=ConstantFunctionComponent input-dim=1 is-updatable=false output-dim=$adapt_dim $prior_std_config
	component-node name=BLHUC.prior_std.$prior_layer component=BLHUC.prior_std.$prior_layer input=feature2
EOF
fi

cat <<EOF >> $dir/configs/change.config	
	component name=BLHUC.mean.$layer type=LinearSelectColComponent input-dim=1 output-dim=$adapt_dim col-num=$spk_num l2-regularize=0.00 param-mean=$param_mean_init param-stddev=0 use-natural-gradient=false learning-rate-factor=${adapt_lr_factor}
...
cat <<EOF >> $dir/configs/change.config	
	component name=BLHUC.std_ori.$layer type=LinearSelectColComponent input-dim=1 output-dim=$tie_dim col-num=$spk_num l2-regularize=0.00 param-mean=$param_std_init param-stddev=0 use-natural-gradient=false learning-rate-factor=${adapt_lr_factor}
...
cat <<EOF >> $dir/configs/change.config	
	...
	component name=BLHUC.std.$layer type=CopyNComponent input-dim=$tie_dim output-dim=$adapt_dim
	component-node name=BLHUC.std.$layer component=BLHUC.std.$layer input=BLHUC.std_scale.$layer
EOF

cat <<EOF >> $dir/configs/change.config	
	component name=BLHUC.vec.$layer type=BayesVecKLGaussianComponent output-dim=$adapt_dim input-dim=$dim_tmp_x4plus1 KL-scale=${KL} input-frame-scale=true inv-frame-scale=true rand-per-frame=false KL-output=false test-mode=false
	component-node name=$adapt_input.$layer$adapt_op component=BLHUC.vec.$layer input=Append(BLHUC.mean.$layer, BLHUC.std.$layer, BLHUC.prior_mean.$prior_layer, BLHUC.prior_std.$prior_layer, BLHUC.count)
EOF
...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
  ...
  steps/nnet3/chain/train_adapt.py --stage $train_stage \
    ...
    ${deriv_truncate_margin:+ --trainer.deriv-truncate-margin $deriv_truncate_margin} \
