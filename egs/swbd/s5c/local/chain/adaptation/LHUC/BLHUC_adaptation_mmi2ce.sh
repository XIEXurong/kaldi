...
cat <<EOF >> $dir/configs/change.config
	component name=no_mmi type=NoOpComponent dim=256 backprop-scale=$mmi_scale
	component-node name=no_mmi component=no_mmi input=prefinal-chain.batchnorm2
	component-node name=output.affine component=output.affine input=no_mmi
    
    component-node name=output-xent.affine component=output.affine input=prefinal-chain.batchnorm2
    
    component name=output-xent.log-softmax type=LogSoftmaxComponent dim=6000
