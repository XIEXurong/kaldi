...
version=_LHUC_transf${adapt_dim}${tag}_adaptlayer${layer_num}_act${act}_batch${num_chunk1}_epoch${epoch_num}_lr1${lr1}_lr2${lr2}
...
adapt_dim=$transf_hidden_dim
fi

cat <<EOF >> $dir/configs/change.config	
	component name=LHUC.transf.$layer type=NaturalGradientAffineComponent input-dim=$adapt_dim output-dim=$dim_tmp  max-change=0.75 l2-regularize=0.004
