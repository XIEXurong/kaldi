...
cat <<EOF >> $dir/configs/change.config
	output-node name=output input=output-xent.affine objective=linear
EOF
...
cat <<EOF > $dir/configs/change1.config
	output-node name=output input=output.affine objective=linear
EOF

  mv $dir/final.mdl $dir/final_ori.mdl
  nnet3-am-copy --binary=false --nnet-config=$dir/configs/change1.config $dir/final_ori.mdl - | \
