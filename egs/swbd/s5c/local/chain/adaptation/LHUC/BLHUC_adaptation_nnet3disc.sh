...
epoch_num=7
lr=0.01
minibatch_size="300=32,16/150=64,32"
param_mean_init=0.0
param_std_init=1.0
...
# training options
frames_per_eg=300,280,150,120,100,50
remove_egs=false
xent_regularize=0.1
...
label_ali_dir=$2 # label_ali_dir=exp/chain/cnn_tdnn_iv_1a_hires_sp/decode_eval2000_hires_sw1_fsh_fg/1BEST_lat/score_10_0.0
label_lat_dir=$3
decode_set=$4 # eval2000_hires_spk

version=_BLHUC${tag}_adaptlayer${layer_num}_act${act}_epoch${epoch_num}_lr${lr}
...
cp -r $dirbase/{final.mdl,tree,phones.txt} $label_ali_dir/
...
if [ -z "$degs_dir" ]; then
  if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
    nj=`cat $label_ali_dir/num_jobs` || exit 1
    steps/nnet3/get_degs_adapt.sh \
      --cmd "$decode_cmd --mem 10G" --nj $nj \
      --self-loop-scale 1.0 --acwt 1.0 \
      --max-copy-jobs $max_copy_jobs --determinize_before_split false \
      --extra-left-context $extra_left_context \
      --extra-right-context $extra_right_context \
      --extra-left-context-initial 0 --extra-right-context-final 0 \
      --stage $get_egs_stage \
      --online-ivector-dir $adapt_ivector_dir \
      --frames-per-eg $frames_per_eg --frames-overlap-per-eg $frames_overlap_per_eg \
      data/${adapt_set} data/lang $dirbase $label_ali_dir $dir/degs $label_lat_dir || exit 1
  fi
  degs_dir=$dir/degs
fi
...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
  ...
  nnet3-am-copy --set-raw-nnet=$dir/0.raw $dir/degs/final.mdl $dir/0.mdl
  
  steps/nnet3/train_discriminative.sh --cmd "$decode_cmd" \
    --stage $train_stage \
    --acoustic-scale 1.0 --num-epochs $epoch_num --boost $boost \
    --effective-lrate $lr --max-param-change $max_param_change \
    --criterion $criterion --drop-frames true \
    --one-silence-class $one_silence_class --minibatch-size "$minibatch_size" \
    --num-jobs-nnet 1 --num-threads 1 \
    --regularization-opts "--xent-regularize=$xent_regularize --l2-regularize=$l2_regularize" \
    --src_model $dir/0.mdl \
    ${degs_dir} $dir || exit 1;
  ...
  nnet3-am-copy --binary=false $dir/final_ori.mdl - | \
   ...
   nnet3-am-copy --binary=true - $dir/final.mdl
