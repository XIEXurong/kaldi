...
if $layer_norm; then
  input_act="act_ln"
cat <<EOF >> $dir/configs/change.config	
	component name=BLHUC.act_ln.$layer type=NormalizeComponent dim=$dim_tmp
	component-node name=BLHUC.act_ln.$layer component=BLHUC.act_ln.$layer input=BLHUC.act.$layer
EOF
fi

cat <<EOF >> $dir/configs/change.config	
	component name=BLHUC.product.$layer type=ElementwiseProductComponent output-dim=$dim_tmp input-dim=$dim_tmp_x2
	component-node name=BLHUC.product.$layer component=BLHUC.product.$layer input=Append($layer.relu, Scale($lhuc_scale, BLHUC.$input_act.$layer))
