...
def get_args():
    ...
    parser.add_argument("--egs.allowed-lengths", type=str, dest='allowed_lengths',
                        default=None,
                        help="""A given file of allowed lengths""")
    # chain options
    ...
    parser.add_argument("--feat-dir", type=str, required=True,
                        ...
                        "the neural network.")
    parser.add_argument("--fst-dir", type=str, required=True,
                        help="""Directory containing the fsts to use for this
                        model.""")
...
def train(args, run_opts):
    ...
    # Check files
    files = ['{0}/feats.scp'.format(args.feat_dir), '{0}/fst.1.scp'.format(args.fst_dir),
             '{0}/num_jobs'.format(args.fst_dir)]
    for file in files:
        if not os.path.isfile(file):
            raise Exception('Expected {0} to exist.'.format(file))

    # Set some variables.
    num_jobs = common_lib.get_number_of_jobs(args.fst_dir)
    ...
    if (args.stage <= -3) and args.egs_dir is None:
        ...
        common_lib.execute_command(
            """steps/nnet3/chain/e2e/get_egs_e2e_noexclude.sh {egs_opts} \
                    --cmd "{command}" \
                    --cmvn-opts "{cmvn_opts}" \
                    ...
                    --srand {srand} \
                    --allowed-lengths-file {allowed_lengths_file} \
                    {data} {dir} {fst_dir} {egs_dir}""".format(
                        ...
                        srand=args.srand,
                        allowed_lengths_file=(args.allowed_lengths if args.allowed_lengths is not None
                                     else '""'),
                        data=args.feat_dir, dir=args.dir, fst_dir=args.fst_dir,
