...
version=_MAPLHN_e2e${tag}_adaptlayer${layer_num}_epoch${epoch_num}_lr1${lr1}_lr2${lr2}
...
cat <<EOF >> $dir/configs/change.config	
	component name=MAPLHN.std.$layer type=LinearSelectColComponent input-dim=1 output-dim=$dim_tmp2 col-num=$spk_num l2-regularize=0.00 param-mean=$param_std_init param-stddev=0 use-natural-gradient=false learning-rate-factor=0
