...
version=_HUB${tag}_adaptlayer${layer_num}_act${act}_batch${num_chunk}_epoch${epoch_num}_lr1${lr1}_lr2${lr2}
...
cat <<EOF >> $dir/configs/change.config	
	component name=HUB.linear.$layer type=LinearSelectColComponent input-dim=1 output-dim=$dim_tmp col-num=$spk_num l2-regularize=0.00 param-mean=$param_init param-stddev=0 use-natural-gradient=false
	component-node name=HUB.linear.$layer component=HUB.linear.$layer input=feature2
	component name=HUB.act.$layer $act_component dim=$dim_tmp
	component-node name=HUB.act.$layer component=HUB.act.$layer input=HUB.linear.$layer
	component name=HUB.sum.$layer type=SumBlockComponent output-dim=$dim_tmp input-dim=$dim_tmp_x2
	component-node name=HUB.sum.$layer component=HUB.sum.$layer input=Append($layer.relu, HUB.act.$layer)
	component-node name=$layer.batchnorm component=$layer.batchnorm input=HUB.sum.$layer
