...
prior_std="0.01 0.01"  # a value or a list of values
common_egs_dir=
...
version=_BHUB${tag}_adaptlayer${layer_num}_act${act}_batch${num_chunk}_epoch${epoch_num}_lr1${lr1}_lr2${lr2}

dirbase=exp/chain/${baseline}
dir=exp/chain/adaptation/HUB/${baseline}${version}
...
cat <<EOF > $dir/configs/change.config
	...
	component name=BHUB.count type=LinearSelectColComponent input-dim=1 output-dim=1 col-num=$spk_num matrix=$spk_count max-change=0 learning-rate-factor=0 l2-regularize=0.00 use-natural-gradient=false
	component-node name=BHUB.count component=BHUB.count input=feature2
EOF


layer_num_minus1=$(awk "BEGIN{print($layer_num-1)}")

act_component="type=NoOpComponent" # Idnt
if [[ "$act" == "Tanh" ]]; then
  act_component="type=TanhComponent self-repair-scale=0"
fi
...
if [[ "$i" < "${#prior_mean_array[*]}" || "$i" < "${#prior_mean_file_array[*]}" || "$i" < "${#prior_std_array[*]}" || "$i" < "${#prior_std_file_array[*]}" ]]; then
  prior_layer=$layer
cat <<EOF >> $dir/configs/change.config
	component name=BHUB.prior_mean.$prior_layer type=ConstantFunctionComponent input-dim=1 is-updatable=false output-dim=$dim_tmp $prior_mean_config
	component-node name=BHUB.prior_mean.$prior_layer component=BHUB.prior_mean.$prior_layer input=feature2
	component name=BHUB.prior_std.$prior_layer type=ConstantFunctionComponent input-dim=1 is-updatable=false output-dim=$dim_tmp $prior_std_config
	component-node name=BHUB.prior_std.$prior_layer component=BHUB.prior_std.$prior_layer input=feature2
EOF
fi

cat <<EOF >> $dir/configs/change.config	
	component name=BHUB.mean.$layer type=LinearSelectColComponent input-dim=1 output-dim=$dim_tmp col-num=$spk_num l2-regularize=0.00 param-mean=$param_mean_init param-stddev=0 use-natural-gradient=false
	component-node name=BHUB.mean.$layer component=BHUB.mean.$layer input=feature2
EOF
...
cat <<EOF >> $dir/configs/change.config	
	component name=BHUB.std_ori.$layer type=LinearSelectColComponent input-dim=1 output-dim=$tie_dim col-num=$spk_num l2-regularize=0.00 param-mean=$param_std_init param-stddev=0 use-natural-gradient=false
	component-node name=BHUB.std_ori.$layer component=BHUB.std_ori.$layer input=feature2
EOF
...
cat <<EOF >> $dir/configs/change.config	
	component name=BHUB.std_scale.$layer type=NoOpComponent dim=$tie_dim backprop-scale=$std_update_scale
	component-node name=BHUB.std_scale.$layer component=BHUB.std_scale.$layer input=BHUB.$std_ori.$layer
	component name=BHUB.std.$layer type=CopyNComponent input-dim=$tie_dim output-dim=$dim_tmp
	component-node name=BHUB.std.$layer component=BHUB.std.$layer input=BHUB.std_scale.$layer
EOF

cat <<EOF >> $dir/configs/change.config	
	component name=BHUB.vec.$layer type=BayesVecKLGaussianComponent output-dim=$dim_tmp input-dim=$dim_tmp_x4plus1 KL-scale=${KL} input-frame-scale=true inv-frame-scale=true rand-per-frame=false KL-output=false test-mode=false
	component-node name=BHUB.vec.$layer component=BHUB.vec.$layer input=Append(BHUB.mean.$layer, BHUB.std.$layer, BHUB.prior_mean.$prior_layer, BHUB.prior_std.$prior_layer, BHUB.count)
	component name=BHUB.act.$layer $act_component dim=$dim_tmp
	component-node name=BHUB.act.$layer component=BHUB.act.$layer input=BHUB.vec.$layer
	component name=BHUB.sum.$layer type=SumBlockComponent output-dim=$dim_tmp input-dim=$dim_tmp_x2
	component-node name=BHUB.sum.$layer component=BHUB.sum.$layer input=Append($layer.relu, BHUB.act.$layer)
	component-node name=$layer.batchnorm component=$layer.batchnorm input=BHUB.sum.$layer
EOF
