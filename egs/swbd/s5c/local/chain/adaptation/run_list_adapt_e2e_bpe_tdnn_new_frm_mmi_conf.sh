...
for decode_set in eval2000 rt03; do
    lab_dir=exp/chain/e2e_tdnnf_7r_bpe3g_mmice/decode_${decode_set}_sw1_fsh_fg/1BEST_lat/score_10_0.0
    mkdir -p ${lab_dir}_best0.8
    num_jobs=`cat ${lab_dir}/num_jobs`
    echo "$num_jobs" > ${lab_dir}_best0.8/num_jobs
    for i in `seq 1 $num_jobs`; do
        lattice-copy --include=exp/chain/e2e_tdnnf_7r_bpe3g_mmice/decode_${decode_set}_sw1_fsh_fg/1BEST_weights/score_10_0.0/weights.sort0.8 \
          "ark:gunzip -c ${lab_dir}/lat.$i.gz|" "ark,t:|gzip -c>${lab_dir}_best0.8/lat.$i.gz"
    done
done
...
bash local/chain/adaptation/LHUC/LHUC_adaptation.sh \
 ...
 --act "Sig" --tag "_eval2000_best0.8_mmice" \
 ...
 exp/chain/e2e_tdnnf_7r_bpe3g_mmice/decode_eval2000_sw1_fsh_fg/1BEST_lat/score_10_0.0_best0.8 \
 eval2000_hires_spk

for decode_set in eval2000_hires_spk; do
    for lm in tg fsh_fg; do
        dir=exp/chain/adaptation/LHUC_e2e/e2e_tdnnf_7r_bpe3g_mmice_LHUC_eval2000_best0.8_mmice_adaptlayer14_actSig_batch64_epoch7_lr10.1_lr20.1/decode_${decode_set}_sw1_${lm}
...
bash local/chain/adaptation/LHUC/LHUC_adaptation.sh \
 ...
 --layer-dim "1536 1536 1536 1536 1536 1536 1536 1536 1536 1536 1536 1536 1536 1536" \
 --input-config "component-node name=input_2 component=input_2 input=Append(Offset(feature1,0), Sum(Offset(Scale(-1.0,input_copy1),-1), Offset(feature1,1)), Sum(Offset(feature1,-2), Offset(feature1,2), Offset(Scale(-2.0,input_copy2),0)))" \
 --act "Sig" --tag "_rt03_best0.8_mmice" \
 --epoch-num 7 --lr1 0.1 --lr2 0.1 --num-chunk 64 --param-init 0.0 \
 --frames-per-eg "150,100,50" \
 --adapt-base adaptation/LHUC_e2e \
 rt03_e2e_hires_spk \
 exp/chain/e2e_tdnnf_7r_bpe3g_mmice/decode_rt03_sw1_fsh_fg/1BEST_lat/score_10_0.0_best0.8 \
 rt03_hires_spk

for decode_set in rt03_hires_spk; do
    for lm in tg fsh_fg; do
        dir=exp/chain/adaptation/LHUC_e2e/e2e_tdnnf_7r_bpe3g_mmice_LHUC_rt03_best0.8_mmice_adaptlayer14_actSig_batch64_epoch7_lr10.1_lr20.1/decode_${decode_set}_sw1_${lm}
        bash compute_score.sh $dir >> $dir/../scoring_all
    done
done
...
for decode_set in eval2000_hires_spk; do
    for lm in tg fsh_fg; do
        dir=exp/chain/adaptation/LHUC_e2e/e2e_tdnnf_7r_bpe3g_mmice_BLHUC_eval2000_best0.8_mmice_adaptlayer14_actSig_batch64_epoch7_lr10.1_lr20.1/decode_${decode_set}_sw1_${lm}
        bash compute_score.sh $dir >> $dir/../scoring_all
    done
done
...
for decode_set in rt03_hires_spk; do
    for lm in tg fsh_fg; do
        dir=exp/chain/adaptation/LHUC_e2e/e2e_tdnnf_7r_bpe3g_mmice_BLHUC_rt03_best0.8_mmice_adaptlayer14_actSig_batch64_epoch7_lr10.1_lr20.1/decode_${decode_set}_sw1_${lm}
        bash compute_score.sh $dir >> $dir/../scoring_all
    done
done
