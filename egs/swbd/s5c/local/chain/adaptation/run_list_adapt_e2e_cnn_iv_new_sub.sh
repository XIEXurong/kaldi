...
for subN in _sub5 _sub10 _sub20 _sub40; do
    bash local/chain/adaptation/LHUC/LHUC_adaptation_e2e.sh --egs_opts "--num_utts_subset 100" \
     ...
     --act "Sig" --tag "_eval2000${subN}_e2ehires" \
     --epoch-num 7 --lr1 0.01 --lr2 0.01 --param-init 0.0 \
     eval2000_e2e_hires_spk${subN} \
     exp/chain/e2e_cnn_tdnnf_1a_iv_bi_mmice_specaugkaldi/decode_eval2000_sw1_fsh_fg/1BEST_fst/score_10_0.0${subN} \
     eval2000_hires_spk

    for decode_set in eval2000_hires_spk; do
        for lm in tg fsh_fg; do
            dir=exp/chain/adaptation/LHUC_e2e/e2e_cnn_tdnnf_1a_iv_bi_mmice_specaugkaldi_LHUC_e2e_eval2000${subN}_e2ehires_adaptlayer7_actSig_epoch7_lr10.01_lr20.01/decode_${decode_set}_sw1_${lm}
...
for subN in _sub5 _sub10 _sub20 _sub40; do
    bash local/chain/adaptation/LHUC/LHUC_adaptation_e2e.sh --egs_opts "--num_utts_subset 100" \
     ...
     --layer-dim "2560 1536 1536 1536 1536 1536 1536" \
     --input-config "component-node name=idct component=idct input=feature1" \
     --act "Sig" --tag "_rt03${subN}_e2ehires" \
     --epoch-num 7 --lr1 0.01 --lr2 0.01 --param-init 0.0 \
     rt03_e2e_hires_spk${subN} \
     exp/chain/e2e_cnn_tdnnf_1a_iv_bi_mmice_specaugkaldi/decode_rt03_sw1_fsh_fg/1BEST_fst/score_10_0.0${subN} \
     rt03_hires_spk

    for decode_set in rt03_hires_spk; do
        for lm in tg fsh_fg; do
            dir=exp/chain/adaptation/LHUC_e2e/e2e_cnn_tdnnf_1a_iv_bi_mmice_specaugkaldi_LHUC_e2e_rt03${subN}_e2ehires_adaptlayer7_actSig_epoch7_lr10.01_lr20.01/decode_${decode_set}_sw1_${lm}
            bash compute_score.sh $dir >> $dir/../scoring_all
        done
    done
done
...
for subN in _sub5 _sub10 _sub20 _sub40; do
    ...
    for decode_set in eval2000_hires_spk; do
        for lm in tg fsh_fg; do
            dir=exp/chain/adaptation/LHUC_e2e/e2e_cnn_tdnnf_1a_iv_bi_mmice_specaugkaldi_BLHUC_e2e_eval2000${subN}_e2ehires_adaptlayer7_actSig_epoch7_lr10.01_lr20.01/decode_${decode_set}_sw1_${lm}
            bash compute_score.sh $dir >> $dir/../scoring_all
        done
    done
done
...
for subN in _sub5 _sub10 _sub20 _sub40; do
    ...
    for decode_set in rt03_hires_spk; do
        for lm in tg fsh_fg; do
            dir=exp/chain/adaptation/LHUC_e2e/e2e_cnn_tdnnf_1a_iv_bi_mmice_specaugkaldi_BLHUC_e2e_rt03${subN}_e2ehires_adaptlayer7_actSig_epoch7_lr10.01_lr20.01/decode_${decode_set}_sw1_${lm}
            bash compute_score.sh $dir >> $dir/../scoring_all
        done
    done
done

result_file=exp/chain/adaptation/LHUC_e2e/results_e2e_cnn_tdnnf_1a_iv_bi_mmice_specaugkaldi_BLHUC_e2e_sub
> $result_file
for decode_set in eval2000 rt03; do
    echo "### ${decode_set}" >> $result_file
    for subN in _sub5 _sub10 _sub20 _sub40 ""; do
        echo "## ${subN}" >> $result_file
        dir=exp/chain/adaptation/LHUC_e2e/e2e_cnn_tdnnf_1a_iv_bi_mmice_specaugkaldi_BLHUC_e2e_${decode_set}${subN}_e2ehires_adaptlayer7_actSig_epoch7_lr10.01_lr20.01
        tail -n 3 $dir/scoring_all >> $result_file
    done
done
