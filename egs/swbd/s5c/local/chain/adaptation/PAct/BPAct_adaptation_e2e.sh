...
deriv_weights_scp=
spk_count_file=
adapt_base=adaptation/PAct_e2e
adapt_model=
init_model=
LM_path=data/lang_
...
version=_BPAct_e2e${tag}_adaptlayer${layer_num}_epoch${epoch_num}_lr1${lr1}_lr2${lr2}
...
if [[ "$i" -lt "${#prior_alpha_mean_array[*]}" || "$i" -lt "${#prior_alpha_mean_file_array[*]}" || "$i" -lt "${#prior_alpha_std_array[*]}" || "$i" -lt "${#prior_alpha_std_file_array[*]}" ]]; then
...
if [[ "$i" -lt "${#prior_beta_mean_array[*]}" || "$i" -lt "${#prior_beta_mean_file_array[*]}" || "$i" -lt "${#prior_beta_std_array[*]}" || "$i" -lt "${#prior_beta_std_file_array[*]}" ]]; then
...
cat <<EOF >> $dir/configs/change.config	
	...
	component name=BPAct.product.$layer type=ElementwiseProductComponent output-dim=$dim_tmp_x2 input-dim=$dim_tmp_x4
	component-node name=BPAct.product.$layer component=BPAct.product.$layer input=Append($layer.relu, Scale(-1.0, $layer.neg_relu), BPAct.alpha.vec.$layer, BPAct.beta.vec.$layer)
	component name=BPAct.sum.$layer type=SumBlockComponent output-dim=$dim_tmp input-dim=$dim_tmp_x2
	component-node name=BPAct.sum.$layer component=BPAct.sum.$layer input=BPAct.product.$layer
	
	component-node name=$layer.batchnorm component=$layer.batchnorm input=BPAct.sum.$layer
EOF
