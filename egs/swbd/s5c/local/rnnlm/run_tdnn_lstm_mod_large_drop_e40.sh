...
dir=exp/rnnlm/rnnlm_lstm_1e_large_drop_e40
embedding_dim=1024
affine_dim=2048
cell_dim=2048
lstm_rpd=512
...
if [ $stage -le 3 ]; then
  rnnlm/train_rnnlm.sh --num-jobs-initial 2 --num-jobs-final 3 --use-gpu "wait" \
...
#  LM=sw1_tg # if using the original 3-gram G.fst as old lm
  ...
  for decode_set in eval2000_hires; do
    ...
    rnnlm/lmrescore$pruned.sh \
      ...
      data/lang_$LM $dir \
      ../s5c_new/data/${decode_set} ${decode_dir} \
      ${decode_dir}_${decode_dir_suffix}_0.45
	  
	> ${decode_dir}_${decode_dir_suffix}_0.45/scoring_all
	grep Sum ${decode_dir}_${decode_dir_suffix}_0.45/score_*/*.ctm.swbd.filt.sys | utils/best_wer.sh >> ${decode_dir}_${decode_dir_suffix}_0.45/scoring_all
	grep Sum ${decode_dir}_${decode_dir_suffix}_0.45/score_*/*.ctm.callhm.filt.sys | utils/best_wer.sh >> ${decode_dir}_${decode_dir_suffix}_0.45/scoring_all
	grep Sum ${decode_dir}_${decode_dir_suffix}_0.45/score_*/*.ctm.filt.sys | utils/best_wer.sh >> ${decode_dir}_${decode_dir_suffix}_0.45/scoring_all
  done
fi
...
if [ $stage -le 5 ] && $run_nbest_rescore; then
  ...
  for decode_set in eval2000_hires; do
	...
	> ${decode_dir}_${decode_dir_suffix}_nbest/scoring_all
	grep Sum ${decode_dir}_${decode_dir_suffix}_nbest/score_*/*.ctm.swbd.filt.sys | utils/best_wer.sh >> ${decode_dir}_${decode_dir_suffix}_nbest/scoring_all
	grep Sum ${decode_dir}_${decode_dir_suffix}_nbest/score_*/*.ctm.callhm.filt.sys | utils/best_wer.sh >> ${decode_dir}_${decode_dir_suffix}_nbest/scoring_all
	grep Sum ${decode_dir}_${decode_dir_suffix}_nbest/score_*/*.ctm.filt.sys | utils/best_wer.sh >> ${decode_dir}_${decode_dir_suffix}_nbest/scoring_all
  done
fi
...
if [ $stage -le 6 ] && $run_backward_rnnlm; then
  local/rnnlm/run_tdnn_lstm_back_large_drop_e40.sh
fi
