...
fisher=/sde/xxr/works/espnet2/egs/swbd_mod/asr1/data/train_fisher/text
...
python3 local/pytorchnn/sort_by_start_time_fisher.py --infile $fisher --outfile $dir/fisher.train.sorted.txt
...
mhm um-hum
EOF
cat $dir/fisher.train.sorted.txt | cut -d ' ' -f2- | tr 'A-Z' 'a-z' | awk 'NR==FNR{a[$1]=$2;next}{for (n=1;n<=NF;n++) if ($n in a) $n=a[$n];print $0}' \
  $dir/config/hesitation_mapping.txt - > $dir/fisher.txt
rm $dir/fisher.train.sorted.txt
