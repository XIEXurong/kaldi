...
for i in `seq 0 $[num_sys-1]`; do
  ...
  for f in ${data}_${i}/feats.scp $model $extra_files; do
...
# split data to nj
[[ -d $sdata && ${data}_${i}/feats.scp -ot $sdata ]] || split_data.sh ${data}_${i} $nj || exit 1;
...
if ! $skip_scoring ; then
  if [ $stage -le 2 ]; then
    ...
    local/score.sh $scoring_opts --cmd "$cmd" ${data}_${i} $graphdir $dir
