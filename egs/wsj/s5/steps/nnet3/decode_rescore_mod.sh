...
old_acoustic_scale=0
new_acoustic_scale=1
...
if [ $stage -le 0 ]; then  
  $cmd --num-threads $num_threads JOB=1:$nj $dir/log/decode.JOB.log \
     ...
     matrix-sum --scale1=$new_acoustic_scale ark:- ark:- \| \
     lattice-rescore-mapped --old-acoustic-scale=$old_acoustic_scale "$model" "ark:gunzip -c $lat_dir/lat.JOB.gz |" ark:- "$lat_wspecifier"
fi
