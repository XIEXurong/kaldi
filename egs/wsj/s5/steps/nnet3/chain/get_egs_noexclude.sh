...
len_uttlist=$(wc -l < $data/utt2spk)
if [ $len_uttlist -lt $(awk "BEGIN{print($num_utts_subset*2)}") ]; then
  num_utts_subset=$(awk "BEGIN{print(int($len_uttlist/2-1))}")
  echo "New num_utts_subset is $num_utts_subset"
  awk '{print $1}' $data/utt2spk | \
	utils/shuffle_list.pl 2>/dev/null | head -$num_utts_subset > $dir/valid_uttlist
fi
...
if [ $len_uttlist -lt $num_utts_subset ]; then
  echo "Number of utterances is very small. Please check your data.";
fi
