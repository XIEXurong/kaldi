...
if [ -z $allowed_lengths_file ]; then
  allowed_lengths_file=$data/allowed_lengths.txt
fi
...
for f in $data/feats.scp ${allowed_lengths_file} \
...
frames_per_eg=$(cat ${allowed_lengths_file} | tr '\n' , | sed 's/,$//')
