...
class XconfigReluLHUCBatchnormDropoutLayer(XconfigLayerBase):
    ...
    Parameters of the class, and their defaults:
      ...
      bottleneck-dim=-1        [If you set this, a linear bottleneck is added, so
                                ...
                                orthonormal.]
      speaker-input='[-1]'     [Input feature name of the speaker ID]
      col-num=-1
      ng-lhuc-options=''
    ...
    def set_default_configs(self):
        ...
        self.config = {'input': '[-1]',
                       ...
                       'speaker-id': '[-1]',
                       'col-num': -1,
                       'lhuc-activation': 'sigmoid', # 0: 2sigmoid; 1: linear; 2: exp
                       'ng-lhuc-options': '',
                       'target-rms': 1.0,
                       'ng-affine-options': '',
    ...
    def _add_components(self, input_desc, input_dim, nonlinearities):
        ...
        lhuc_activation = self.config['lhuc-activation']
        lhuc_options = self.config['ng-lhuc-options']
        ...
        for i, nonlinearity in enumerate(nonlinearities):
            if nonlinearity == 'relu':
                ...
                if not (self.config['dropout-per-dim'] or
                    ...
                    line = ('component name={0}.dropout type=GeneralDropoutComponent '
                            'dim={1} dropout-proportion={2} {3}'.format(
                                ...
                                continuous_opt))
            elif nonlinearity == 'lhuc':
                line = ('component name={0}.LHUC.linear type=LinearSelectColComponent '
                        'input-dim=1 output-dim={1} col-num={2} {3}'.format(self.name, output_dim, col_num, lhuc_options))
                configs.append(line)
                line = ('component-node name={0}.LHUC.linear component={0}.LHUC.linear input={1}'.format(self.name, speaker_input))
                configs.append(line)
                if lhuc_activation == 'sigmoid':
                    line = ('component name={0}.LHUC.sigmoid type=SigmoidComponent dim={1} self-repair-scale=0'.format(self.name, output_dim))
                    configs.append(line)
                    line = ('component-node name={0}.LHUC.sigmoid component={0}.LHUC.sigmoid input={0}.LHUC.linear'.format(self.name))
                    configs.append(line)
                elif lhuc_activation == 'linear':
                    line = ('component name={0}.LHUC.noop type=NoOpComponent dim={1}'.format(self.name, output_dim))
                    configs.append(line)
                    line = ('component-node name={0}.LHUC.noop component={0}.LHUC.noop input={0}.LHUC.linear'.format(self.name))
                    configs.append(line)
                elif lhuc_activation == 'exp':
                    line = ('component name={0}.LHUC.exp type=ExpComponent dim={1} self-repair-scale=0'.format(self.name, output_dim))
                    configs.append(line)
                    line = ('component-node name={0}.LHUC.exp component={0}.LHUC.exp input={0}.LHUC.linear'.format(self.name))
                    configs.append(line)
                ...
                configs.append(line)
                if lhuc_activation == 'sigmoid':
                    line = ('component-node name={0}.product component={0}.product input=Append({1}, Scale(2.0, {0}.LHUC.sigmoid))'.format(self.name, cur_node))
                elif lhuc_activation == 'linear':
                    line = ('component-node name={0}.product component={0}.product input=Append({1}, {0}.LHUC.noop)'.format(self.name, cur_node))
                elif lhuc_activation == 'exp':
                    line = ('component-node name={0}.product component={0}.product input=Append({1}, {0}.LHUC.exp)'.format(self.name, cur_node))
                configs.append(line)
...
class XconfigTdnnfLHUCLayer(XconfigLayerBase):

    def __init__(self, first_token, key_to_value, prev_names = None):
        assert first_token == "tdnnf-lhuc-layer"
        XconfigLayerBase.__init__(self, first_token, key_to_value, prev_names)
    ...
    def _generate_config(self):
        ...
        configs.append('component name={0}.LHUC.linear type=LinearSelectColComponent input-dim=1 output-dim={1} col-num={2} {3}'.format(name, output_dim, col_num, lhuc_options))
        configs.append('component-node name={0}.LHUC.linear component={0}.LHUC.linear input={1}'.format(name, speaker_input))
        if lhuc_activation == 'sigmoid':
            configs.append('component name={0}.LHUC.sigmoid type=SigmoidComponent dim={1} self-repair-scale=0'.format(name, output_dim))
            configs.append('component-node name={0}.LHUC.sigmoid component={0}.LHUC.sigmoid input={0}.LHUC.linear'.format(name))
        elif lhuc_activation == 'linear':
            configs.append('component name={0}.LHUC.noop type=NoOpComponent dim={1}'.format(name, output_dim))
            configs.append('component-node name={0}.LHUC.noop component={0}.LHUC.noop input={0}.LHUC.linear'.format(name))
        elif lhuc_activation == 'exp':
            configs.append('component name={0}.LHUC.exp type=ExpComponent dim={1} self-repair-scale=0'.format(name, output_dim))
            configs.append('component-node name={0}.LHUC.exp component={0}.LHUC.exp input={0}.LHUC.linear'.format(name))
        ...
        if lhuc_activation == 'sigmoid':
            configs.append('component-node name={0}.product component={0}.product input=Append({0}.relu, Scale(2.0, {0}.LHUC.sigmoid))'.format(name))
        elif lhuc_activation == 'linear':
            configs.append('component-node name={0}.product component={0}.product input=Append({0}.relu, {0}.LHUC.noop)'.format(name))
        elif lhuc_activation == 'exp':
            configs.append('component-node name={0}.product component={0}.product input=Append({0}.relu, {0}.LHUC.exp)'.format(name))
