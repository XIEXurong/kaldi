...
class XconfigReluBLHUCUntiedBatchnormDropoutLayer(XconfigLayerBase):
		...
		for i, nonlinearity in enumerate(nonlinearities):
			if nonlinearity == 'relu':
				...
				line = ('component name={0}.BLHUC.std1 type=LinearSelectColComponent '
						'input-dim=1 output-dim={1} col-num={2} {3}'.format(self.name, output_dim, col_num, blhuc_std_options))
				configs.append(line)
				line = ('component-node name={0}.BLHUC.std1 component={0}.BLHUC.std1 input={1}'.format(self.name, speaker_input))
				configs.append(line)
				line = ('component name={0}.BLHUC.std type=NoOpComponent '
						'dim={1} backprop-scale={2}'.format(self.name, output_dim, tied_std_scale))
				configs.append(line)
				line = ('component-node name={0}.BLHUC.std component={0}.BLHUC.std input={0}.BLHUC.std1'.format(self.name))
				configs.append(line)
...
class XconfigReluBLHUCUntied1BatchnormDropoutLayer(XconfigLayerBase):
	def __init__(self, first_token, key_to_value, prev_names=None):
		XconfigLayerBase.__init__(self, first_token, key_to_value, prev_names)
				...
				if not (self.config['dropout-per-dim'] or
					...
					line = ('component name={0}.dropout type=GeneralDropoutComponent '
							'dim={1} dropout-proportion={2} {3}'.format(
								self.name, output_dim, self.config['dropout-proportion'],
								continuous_opt))
			elif nonlinearity == 'blhucuntied1':
...
class XconfigTdnnfBLHUCUntiedLayer(XconfigLayerBase):
	...
	def _generate_config(self):
		...
		configs.append('component name={0}.BLHUC.std1 type=LinearSelectColComponent input-dim=1 output-dim={1} col-num={2} {3}'.format(name, output_dim, col_num, blhuc_std_options))
		configs.append('component-node name={0}.BLHUC.std1 component={0}.BLHUC.std1 input={1}'.format(name, speaker_input))
		configs.append('component name={0}.BLHUC.std type=NoOpComponent dim={1} backprop-scale={2}'.format(name, output_dim, tied_std_scale))
		configs.append('component-node name={0}.BLHUC.std component={0}.BLHUC.std input={0}.BLHUC.std1'.format(name))
