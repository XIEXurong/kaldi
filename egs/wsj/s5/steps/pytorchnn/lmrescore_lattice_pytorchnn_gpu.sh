...
if [ $stage -le 2 ]; then
  ...
  mkdir -p $dir/tmp
  if [ "$gpu_nj" == "1" ]; then
      cat $paths/path.*.words_text > $dir/tmp/path.words_text
      
      $cuda_cmd $dir/log/compute_sentence_scores.log $limit_num_gpus_cmd \
        python3 steps/pytorchnn/compute_sentence_scores_gpu.py \
         --infile $dir/tmp/path.words_text \
         --outfile $dir/tmp/${model_type}_scores \
         --vocabulary $vocab \
         --model-path $nn_model \
         --model $model_type \
         ...
         --nlayers $nlayers \
         --nhead $nhead \
         --oov "$oov_symbol" --cuda $tied_opt ${gpu_wait:+ --gpu_wait} || exit 1;
      ...
      for i in `seq 1 $gpu_nj`; do
        ...
        for j in `seq $id_start $id_end`; do
          cat $paths/path.$j.words_text >> $dir/tmp/path.$i.words_text
        done
        $cuda_cmd $dir/log/compute_sentence_scores.$i.log $limit_num_gpus_cmd \
            python3 steps/pytorchnn/compute_sentence_scores_gpu.py \
             --infile $dir/tmp/path.$i.words_text \
             --outfile $dir/tmp/${model_type}_scores.$i \
             --vocabulary $vocab \
             --model-path $nn_model \
             --model $model_type \
             --emsize $embedding_dim \
      ...
      cat $dir/tmp/${model_type}_scores.* > $dir/tmp/${model_type}_scores
  fi
   
  for i in `seq 1 $nj`; do
    perl $scripts/find_pdf.pl $dir/tmp/${model_type}_scores $paths/path.$i.words_text > $scores/${model_type}_scores.$i
  done
  rm -r $dir/tmp
fi
