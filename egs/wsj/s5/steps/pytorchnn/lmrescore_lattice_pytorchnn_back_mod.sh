...
if [ $stage -le 1 ]; then
  ...
  for n in $(seq $nj); do
    ...
    cat $paths/path.$n.words | awk '{printf("%s ",$1);for(i=NF;i>1;i--) printf("%s ",$i); print""}' > $paths/path.$n.words_inv
    utils/int2sym.pl -f 2- $oldlang/words.txt < $paths/path.$n.words_inv > $paths/path.$n.words_text_inv || exit 1;
  done
fi
...
if [ $stage -le 2 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/compute_sentence_scores.JOB.log \
    PYTHONPATH=steps/pytorchnn python3 steps/pytorchnn/compute_sentence_scores.py \
     --infile $paths/path.JOB.words_text_inv \
     --outfile $scores/${model_type}_scores_inv.JOB \
     --vocabulary $vocab \
     --model-path $nn_model \
     --model $model_type \
  ...
  for n in $(seq $nj); do
    cat $scores/${model_type}_scores_inv.$n | awk '{printf("%s ",$1);for(i=NF;i>1;i--) printf("%s ",$i); print""}' > $scores/${model_type}_scores.$n
  done
fi
