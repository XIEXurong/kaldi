...
parser.add_argument('--wer_max', type=float, default=0.2,
                    help='Maximum wer for training')
...
def add_wer(data_ori, ntokens, wer_max):
    data_size = data_ori.size()
    rand_pos = torch.rand(data_size).to(device)
    wer_pos = (rand_pos<wer_max)*1
    true_pos = 1 - wer_pos
    rand_val = torch.randint_like(data_ori, 0,ntokens)
    wer_val = rand_val*wer_pos
    true_val = data_ori*true_pos
    data = true_val+wer_val
    return data
...
def train():
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.seq_len)):
        ...
        data = add_wer(data, ntokens, args.wer_max)
        optimizer.zero_grad()
        if args.model == 'Transformer':
            output = model(data)
        else:
