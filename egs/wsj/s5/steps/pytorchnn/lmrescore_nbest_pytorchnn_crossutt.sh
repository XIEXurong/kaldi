...
if [ $stage -le 6 ]; then
  ...
  if [ -z $conv_sort_list ]; then
      $cmd JOB=1:$nj $dir/log/compute_sentence_scores.JOB.log \
        PYTHONPATH=steps/pytorchnn python3 steps/pytorchnn/compute_sentence_scores_crossutt.py \
            ...
            --nhead $nhead \
            --seq_len $seq_len \
            --oov "$oov_symbol" $tied_opt $reset_history_opt
  else
      mkdir -p ${adir}_tmp
      > ${adir}_tmp/words_text
      for utt in `cat $conv_sort_list | awk '{print $1}'`; do
        cat $adir.*/words_text | grep $utt >> ${adir}_tmp/words_text
      done
      $cmd $dir/log/compute_sentence_scores.JOB.log \
        PYTHONPATH=steps/pytorchnn python3 steps/pytorchnn/compute_sentence_scores_crossutt.py \
            --infile ${adir}_tmp/words_text \
            ...
            --nhead $nhead \
            --seq_len $seq_len \
            --oov "$oov_symbol" $tied_opt $reset_history_opt --conversation_history
      for i in `seq 1 $nj`; do
        perl $scripts/find_pdf.pl ${adir}_tmp/lmwt.nn $adir.$i/words_text > $adir.$i/lmwt.nn
      done
      rm -r ${adir}_tmp
  fi
fi
