...
def get_input_and_target(args, hyps, vocab, device):
    ...
    return data.to(device), target.to(device), seq_lens
...
def compute_scores(args, sents, model, criterion, ntokens, vocab, device, model_type='LSTM'):
    ...
    for idx, key in enumerate(sents.keys()):
        ...
        data, targets, seq_lens = get_input_and_target(args, sents[key], vocab, device)
        if model_type != 'Transformer':
            if idx == 0:
