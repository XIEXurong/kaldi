...
for data in train valid test; do
  cat $dir/swbd.${data}.sorted.txt | cut -d ' ' -f2- | tr 'A-Z' 'a-z' | awk '{for(i=NF;i>0;i--) printf("%s ", $i); print""}' > $dir/$data.txt
...
gunzip -c $fisher | awk 'NR==FNR{a[$1]=$2;next}{for (n=1;n<=NF;n++) if ($n in a) $n=a[$n];print $0}' \
  $dir/config/hesitation_mapping.txt - | awk '{for(i=NF;i>0;i--) printf("%s ", $i); print""}' > $dir/fisher.txt
