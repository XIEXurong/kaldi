...
KL=$2
decode_lat_dir=$3
version=_BLHUC_${layer_num}Layergap7_new_rt02_rnnlmplus${splitn}_KL${KL}_increase${tag}_batch${num_chunk}_epoch${epoch_num}_lr1${lr1}_lr2${lr2}
if [ -e data/rt02 ]; then maybe_rt02=rt02; else maybe_rt02= ; fi
common_egs_dir=
...
if [ -z $decode_lat_dir ]; then
	decode_lat_dir=cnn_tdnn_fbk_40_iv_1a_ep6_multijobs_sp/decode_rt02_fbk_40_sw1_fsh_fg_rnnlm_1e_back_large_drop_e40_0.45/1BEST_lat/score_10_0.0
fi

train_set=rt02_fbk_40_spk$splitn
label_lat_dir=exp/chain/old_models/$decode_lat_dir
spk_count=data/$train_set/spk_count
...
if [ $stage -le 13 ]; then
  local/chain/adaptation/train_adapt.py --stage $train_stage \
    --cmd "$train_cmd" \
	--feat.online-ivector-dir exp/nnet3/old_models/ivectors_rt02 \
...
if [ $stage -le 14 ]; then
  ...
  # the lang directory.
  utils/mkgraph.sh --self-loop-scale 1.0 data/old_models/lang_sw1_tg $dir $dir/graph_sw1_tg
fi
...
if [ $stage -le 15 ]; then
  ...
  for decode_set in rt02_fbk_40_spk; do
      (
...
> $dir/scoring_all
grep Sum $dir/decode_rt02_fbk_40_spk_sw1_tg/score_*/*.ctm.swbd1.filt.sys | utils/best_wer.sh >> $dir/scoring_all
grep Sum $dir/decode_rt02_fbk_40_spk_sw1_tg/score_*/*.ctm.swbd2.filt.sys | utils/best_wer.sh >> $dir/scoring_all
grep Sum $dir/decode_rt02_fbk_40_spk_sw1_tg/score_*/*.ctm.swbdc.filt.sys | utils/best_wer.sh >> $dir/scoring_all
grep Sum $dir/decode_rt02_fbk_40_spk_sw1_tg/score_*/*.ctm.filt.sys | utils/best_wer.sh >> $dir/scoring_all

grep Sum $dir/decode_rt02_fbk_40_spk_sw1_fsh_fg/score_*/*.ctm.swbd1.filt.sys | utils/best_wer.sh >> $dir/scoring_all
grep Sum $dir/decode_rt02_fbk_40_spk_sw1_fsh_fg/score_*/*.ctm.swbd2.filt.sys | utils/best_wer.sh >> $dir/scoring_all
grep Sum $dir/decode_rt02_fbk_40_spk_sw1_fsh_fg/score_*/*.ctm.swbdc.filt.sys | utils/best_wer.sh >> $dir/scoring_all
grep Sum $dir/decode_rt02_fbk_40_spk_sw1_fsh_fg/score_*/*.ctm.filt.sys | utils/best_wer.sh >> $dir/scoring_all
